...
def main(args):
    ...
    if args.model in sew_resnet.__dict__:
        model = sew_resnet.__dict__[args.model](
            pretrained=False, cnf='ADD', spiking_neuron=neuron.IFNode,
            surrogate_function=surrogate.ATan(), T=args.T, step_mode='m', backend='cupy')
    else:
    ...
    if args.olts:
        def olts_loss(y, target):
            return F.cross_entropy(y[-1], target)
...
def parse_args():
    ...
    parser.add_argument('--olts', action='store_true', help='use the olts loss')
